...
polyhedron::polyhedron(char *name)
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        plane this_plane=this->get_plane_by_two_edges(edges_list[edges_num[0]],edges_list[edges_num[2]]);
...
std::vector<int> polyhedron::get_facet_nums_by_edge_num(int edge_num) const
{
    std::vector<int> result_vector;
    int len1=(int)facets_list.size();
    for(int i=0;i<len1;i++)
    {
        std::vector<int> cur_edges=facets_list[i].edges;
        int len2=(int)cur_edges.size();
        for(int j=0;j<len2;j++)
        {
            if(edge_num==cur_edges[j])
            {
